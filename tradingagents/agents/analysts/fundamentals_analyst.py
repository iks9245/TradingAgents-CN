...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        system_prompt = (
            ...
            "✅ 工作流程："
            "1. 【第一次調用】如果消息歷史中沒有工具結果（ToolMessage），立即調用 get_stock_fundamentals_unified 工具"
            "2. 【收到數據後】如果消息歷史中已經有工具結果（ToolMessage），🚨 絕對禁止再次調用工具！🚨"
            "3. 【生成報告】收到工具數據後，必須立即生成完整的基本面分析報告，包含："
            "   - 公司基本信息和財務數據分析"
            "   - PE、PB、PEG等估值指標分析"
            "   - 當前股價是否被低估或高估的判斷"
            "   - 合理價位區間和目標價位建議"
            "   - 基於基本面的投資建議（買入/持有/賣出）"
            "4. 🚨 重要：工具只需調用一次！一次調用返回所有需要的數據！不要重複調用！🚨"
            "5. 🚨 如果你已經看到ToolMessage，說明工具已經返回數據，直接生成報告，不要再調用工具！🚨"
            "可用工具：{tool_names}。\n{system_message}"
            "當前日期：{current_date}。"
        ...
        if GoogleToolCallHandler.is_google_model(fresh_llm):
            ...
            if current_tool_calls > 0:
                ...
                if has_tool_result:
                    ...
                    force_system_prompt = (
                        f"你是專業的股票基本面分析師。"
                        f"你已經收到了股票 {company_name}（代碼：{ticker}）的基本面數據。"
                        f"🚨 現在你必須基於這些數據生成完整的基本面分析報告！🚨\n\n"
                        f"報告必須包含以下內容：\n"
                        f"1. 公司基本信息和財務數據分析\n"
                        f"2. PE、PB、PEG等估值指標分析\n"
                        f"3. 當前股價是否被低估或高估的判斷\n"
                        f"4. 合理價位區間和目標價位建議\n"
                        f"5. 基於基本面的投資建議（買入/持有/賣出）\n\n"
                        f"要求：\n"
                        f"- 使用中文撰寫報告\n"
                        f"- 基於消息歷史中的真實數據進行分析\n"
                        f"- 分析要詳細且專業\n"
                    ...
                    fallback_report = f"基本面分析（股票代碼：{ticker}）\n\n由於達到最大工具調用次數限制，使用簡化分析模式。建議檢查數據源連接或降低分析複雜度。"
                    return {
                        "messages": [result],
